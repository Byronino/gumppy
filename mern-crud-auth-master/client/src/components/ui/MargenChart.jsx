...
export const MargenChart = ({diente,parametro})=>{
    const getValue = (array, index, numero) => {
      ...
      const data1 = parametro.map((examen, index) => {
        if (diente < 8) {
          return { name: `${index + 1}`, uv: getValue(examen.mar1i, diente, 0) };
        } else if (diente < 16) {
          return { name: `${index + 1}`, uv: getValue(examen.mar1d, diente % 8, 0) };
        } else if (diente < 24) {
          return { name: `${index + 1}`, uv: getValue(examen.mar3i, diente % 8, 0) };
        } else {
          return { name: `${index + 1}`, uv: getValue(examen.mar3d, diente % 8, 0) };
        }
      });
      
      const data2 = parametro.map((examen, index) => {
        if (diente < 8) {
          return { name: `${index + 1}`, uv: getValue(examen.mar1i, diente, 1) };
        } else if (diente < 16) {
          return { name: `${index + 1}`, uv: getValue(examen.mar1d, diente % 8, 1) };
        } else if (diente < 24) {
          return { name: `${index + 1}`, uv: getValue(examen.mar3i, diente % 8, 1) };
        } else {
          return { name: `${index + 1}`, uv: getValue(examen.mar3d, diente % 8, 1) };
        }
      });
      
      const data3 = parametro.map((examen, index) => {
        if (diente < 8) {
          return { name: `${index + 1}`, uv: getValue(examen.mar1i, diente, 2) };
        } else if (diente < 16) {
          return { name: `${index + 1}`, uv: getValue(examen.mar1d, diente % 8, 2) };
        } else if (diente < 24) {
          return { name: `${index + 1}`, uv: getValue(examen.mar3i, diente % 8, 2) };
        } else {
          return { name: `${index + 1}`, uv: getValue(examen.mar3d, diente % 8, 2) };
        }
      });
      
      const data4 = parametro.map((examen, index) => {
        if (diente < 8) {
          return { name: `${index + 1}`, uv: getValue(examen.mar2i, diente, 0) };
        } else if (diente < 16) {
          return { name: `${index + 1}`, uv: getValue(examen.mar2d, diente % 8, 0) };
        } else if (diente < 24) {
          return { name: `${index + 1}`, uv: getValue(examen.mar4i, diente % 8, 0) };
        } else {
          return { name: `${index + 1}`, uv: getValue(examen.mar4d, diente % 8, 0) };
        }
      });
      
      const data5 = parametro.map((examen, index) => {
        if (diente < 8) {
          return { name: `${index + 1}`, uv: getValue(examen.mar2i, diente, 1) };
        } else if (diente < 16) {
          return { name: `${index + 1}`, uv: getValue(examen.mar2d, diente % 8, 1) };
        } else if (diente < 24) {
          return { name: `${index + 1}`, uv: getValue(examen.mar4i, diente % 8, 1) };
        } else {
          return { name: `${index + 1}`, uv: getValue(examen.mar4d, diente % 8, 1) };
        }
      });
      
      const data6 = parametro.map((examen, index) => {
        if (diente < 8) {
          return { name: `${index + 1}`, uv: getValue(examen.mar2i, diente, 2) };
        } else if (diente < 16) {
          return { name: `${index + 1}`, uv: getValue(examen.mar2d, diente % 8, 2) };
        } else if (diente < 24) {
          return { name: `${index + 1}`, uv: getValue(examen.mar4i, diente % 8, 2) };
        } else {
          return { name: `${index + 1}`, uv: getValue(examen.mar4d, diente % 8, 2) };
        }
      });
    return(
        <WhiteWindow>
